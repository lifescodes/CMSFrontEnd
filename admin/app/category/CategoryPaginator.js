(function(){
  ...
  angular
  .module('app.post')
  .service('CategoryPaginator', CategoryPaginator);

  CategoryPaginator.$inject = ['CategoryService','logger'];

  function CategoryPaginator(CategoryService, logger){
    ...
    function executeRest(curr) {
      ...
      var csl = CategoryService.list(1, 1000);
      if (curr !== undefined) {
        csl = CategoryService.list(1, 1000);
        current(curr);
      }
      csl.then(
        function(response) {
      ...
      var cst = CategoryService.tree();
      if (curr !== undefined) {
        cst = CategoryService.tree(curr, limit());
        current(curr);
      }
      ...
      cst.then(
        function(response) {
          ...
          count(response.meta.count); // pagination
          return response;
        },
        function(error) {
          console.log(error);
